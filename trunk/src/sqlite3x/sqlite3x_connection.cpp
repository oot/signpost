...
bool sqlite3_connection::existtable(const char *table) {
	if(!this->db) throw database_error("database is not open");

	std::string sql("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='");
	sql.append(table);
	sql.append("'");
	int result = sqlite3_command(*this, sql.c_str()).executeint();

	return (result > 0);
}

bool sqlite3_connection::existtable(const wchar_t *table) {
	if(!this->db) throw database_error("database is not open");

	std::wstring sql(L"SELECT count(*) FROM sqlite_master WHERE type='table' AND name='");
	sql.append(table);
	sql.append(L"'");
	int result = sqlite3_command(*this, sql.c_str()).executeint();
